...
public abstract class ESTRequestAuthorizer {
   ...
   protected static void ensureCSRMatchesToBeCert(PKCS10 csr, User user)
       ...
       String subject = csr.getSubjectName().toString().substring(3);

       if (!StringUtils.equalsIgnoreCase(subject, user.getFullName()) &&
               !StringUtils.equalsIgnoreCase(subject, user.getUserID())) {
           throw new ForbiddenException("CSR subject does not match user identity.");
       }
       ...
       if (csrSAN != null) {
           for(GeneralNameInterface gName: csrSAN.getGeneralNames()) {
               ...
               throw new ForbiddenException(
                       "CSR SubjectAlternativeName extension does not match usser."
                       );
               }
           }
       }
