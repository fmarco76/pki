...
public class OCSPApplication extends Application {
    ...
    private static String getDefaultV1ApiStatus() {
        try (InputStream is = OCSPApplication.class.getClassLoader().getResourceAsStream("build.properties")) {
            if (is != null) {
            ...
            logger.warn("OCSPApplication: Unable to read build.properties, defaulting to enabled", e);
        }
        return "enabled";
    }
    ...
    public OCSPApplication() {
        ...
        if ("disabled".equals(v1ApiStatus)) {
            logger.warn("======================================================================");
            logger.warn("OCSP v1 REST API has been DISABLED.");
