...
public class TKSApplication extends Application {
    ...
    private static String getDefaultV1ApiStatus() {
        try (InputStream is = TKSApplication.class.getClassLoader().getResourceAsStream("build.properties")) {
            if (is != null) {
            ...
            logger.warn("TKSApplication: Unable to read build.properties, defaulting to enabled", e);
        }
        return "enabled";
    }
    ...
    public TKSApplication() {
        ...
        if ("disabled".equals(v1ApiStatus)) {
            logger.warn("======================================================================");
            logger.warn("TKS v1 REST API has been DISABLED.");
